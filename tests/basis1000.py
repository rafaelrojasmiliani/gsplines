...
import sympy as sp
import unittest
from gsplines.basis.basis1000 import cBasis1000
...
class cMyTest(unittest.TestCase):
    ...
    def test_l2_norms(self):
        ...
        Bimpl = cBasis1000()
        bdim = Bimpl.dim_
        Qd0 = np.zeros((bdim, bdim))
        Qd1 = np.zeros((bdim, bdim))
        Bimpl.l2_norm(tau, Qd0, 0)
        Bimpl.l2_norm(tau, Qd1, 1)
        y = np.random.rand(Bimpl.dim_)

        def qd0norm2(s):
            res = Bimpl.evalDerivOnWindow(s, tau, 0).dot(y)
        ...
        dt = 5.0e-6
        for f, Q in [(qd0norm2, Qd0), (qd1norm2, Qd1)]:
