...
class cDistFunction(object):
    ...
    def getTimes(self, tauv):
        ...
        return self.timebuffer_, self.timebuffer2_, self.timebuffer3_
    ...
    def __call__(self, _tauv, _wp):
        ...
        tis, sis, iintrs = self.getTimes(_tauv)
    ...
    def jacobian(self, _tauv, _wp, _wpidx=[], _dydwbuff=None, _resbuff=None):
        ...
        q = q(tis)
        qd = qd(tis)
        for i, (ti, si, iintr_0) in enumerate(zip(tis, sis, iintrs)):
            dgdq_ti = g.deriv_wrt_q(q[i, :], qd[i, :])
            dgdqd_ti = g.deriv_wrt_qd(q[i, :], qd[i, :])

            dq_dtaui = self.qbuff1_
            dqd_dtaui = self.qbuff2_
            for iintr, taui in enumerate(_tauv):
                for idim in range(dim):
                    i0 = iintr * 6 * dim + 6 * idim
                    i1 = i0 + 6
                    dyidtaui = dydtau[i0:i1, iintr]
                    yi = y[i0:i1]
                    B = basis.evalOnWindow(si, taui)
                    dq_dtaui[idim] = dyidtaui.dot(B)

                    if iintr == iintr_0:
                        B = basis.evalDerivWrtTauOnWindow(si, taui, 0)
                        dq_dtaui[idim] += yi.dot(B)

                    Bd = basis.evalDerivOnWindow(si, taui, 1)
                    dqd_dtaui[idim] = dydtau[i0:i1, iintr].dot(Bd)

                    if iintr == iintr_0:
                        B = basis.evalDerivWrtTauOnWindow(si, taui, 1)
                        dqd_dtaui[idim] += yi.dot(B)

                _resbuff[i, iintr] = dgdq_ti.dot(dq_dtaui) \
                    + dgdqd_ti.dot(dqd_dtaui)
            ...
            for i_u in range(ulen):
                ...
                _resbuff[i, i_u + N] = dgdq_ti.dot(dq_dui)
